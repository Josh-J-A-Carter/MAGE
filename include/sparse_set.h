...
struct Page {
	...
	DenseIndex operator[](PageIndex page_index) const {
...
private:
	std::vector<Page> sparse_entries;
	mutable std::vector<T> dense_entries;
	std::unordered_map<DenseIndex, SparseIndex> dense_to_sparse;

	std::size_t page_size;
	std::size_t max_count;

	std::pair<PageIndex, PageIndex> sparse_to_page_indices(SparseIndex sparse_index) const {
		...
		assert(page_number < sparse_entries.size() && "Sparse index maps to non-existent page");
...
public:
	SparseSet(std::size_t max_count, std::size_t page_size = Page::default_size)
		...
		sparse_entries ( max_count / page_size + (max_count % page_size == 0 ? 0 : 1) ) {

		// This reserve size is rather arbitrary - should be adjusted as needed
		dense_to_sparse.reserve(page_size);
	}

	T& operator[](SparseIndex sparse_index) const {
		auto [ page_number, page_index ] = sparse_to_page_indices(sparse_index);

		DenseIndex dense_index = sparse_entries[page_number][page_index];

		assert(dense_index != Page::invalid_index && "Attempted to access a non-existent entry");

		return dense_entries[dense_index];
	}

	T& dense(DenseIndex dense_index) const {
		assert(dense_index < dense_entries.size() && "Attempted to access beyond the end of the dense array");

		return dense_entries[dense_index];
	}

	std::size_t size() const {
		return dense_entries.size();
	}

	void add_entry(SparseIndex sparse_index, T entry) {
		auto [ page_number, page_index ] = sparse_to_page_indices(sparse_index);

		// There is already an associated entry
		if (sparse_entries[page_number][page_index] != Page::invalid_index) return;

		DenseIndex dense_index = dense_entries.size();
		dense_entries.emplace_back(entry);
		sparse_entries[page_number].add_entry(page_index, dense_index, page_size);
	...
	void remove_entry(SparseIndex sparse_index0) {
		...
		DenseIndex dense_index0 = sparse_entries[page_number0][page_index0];

		// There is no associated sparse entry
		if (dense_index0 == Page::invalid_index) return;

		assert(dense_entries.size() > 0 && "Attempted to remove entry, but no entries are currently stored.");

		// Find the sparse index of the last dense entry
		DenseIndex dense_index1 = dense_entries.size() - 1;
		...
		dense_entries[dense_index0] = dense_entries[dense_index1];
		dense_entries.pop_back();
		...
		sparse_entries[page_number0].remove_entry(page_index0);
		sparse_entries[page_number1].remove_entry(page_index1);
